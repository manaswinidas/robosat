...
Notes:
- See `rs weights` for class weight calculation and `rs stats` for mean and std dev calculation.
- If you run `rs train` in an environment without X11 you need to set `export MPLBACKEND="agg"` for charts, see [the matplotlib docs](https://matplotlib.org/faq/howto_faq.html#matplotlib-in-a-web-application-server).
